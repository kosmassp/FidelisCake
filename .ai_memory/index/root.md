...
| `version.txt` | The update manifest tills read from `raw.githubusercontent.com/.../master/version.txt` — `Version:` / `Drive:` / `File:` / `Sha256:` / `Notes:` lines parsed by `Business/UpdateManifest.cs`. Edited **after** the matching GitHub release is published, never before; editing it is what arms a rollout. |
| `.github/workflows/release.yml` | Tag push `v*` → verifies the tag matches `AssemblyVersion`, builds (CI retargets to 4.6.2 — hosted runners lack the 4.6 reference assemblies), zips the build output minus `exe.config`, publishes the release with the ZIP and its SHA-256 (plus a paste-ready `version.txt` block) in the body. Never touches `version.txt` itself. |
